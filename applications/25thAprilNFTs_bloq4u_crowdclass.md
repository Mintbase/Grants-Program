...
The Crowdclass platform, which the Fiat-to-Mint & Token-gating applications will be integrated with, is currently in Private Beta and under migration to NEAR Protocol. The migration will be complete before the end of 2022, and they will launch the public version in 2023.

Its core functionality already includes the following:
- An authoring tool to make it easy to create NFT digital badges or certificates;
- An NFT distribution model via e-mail or a QR code link;
- An NFT claim-n-mint system that hides all the complexities of Web3 that onboards newcomers and new users smoothly into Web3;
- Several reports & analytics to check the performance of a specific initiative;
- Public pages to view and share the NFTs in social media and other means.
...
### Milestone 1 — Build Fiat-to-Mint storefront
...
| 1. | Buyers Front-End | The primary way users will interact with the application is to buy an NFT. According to the philosophy used in Crowdclass Platform, we will mainly focus this front-end on regular internet users and non-web3 natives |  
| 2. | Collectibles Management | A back-office & admin tool to manage all the collectibles for sale. Includes uploading images or video, adding metadata, setting the price, creator royalties, and so on. |  
| 3. | Paywall & Processor | Deployment of a paywall using Stripe or another payment processor to be able to receive funds |  
| 4. | NFT Minter & Airdropper | This component will mint NFTs through the MintbaseJS minter directly to a connected NEAR Wallet or send them to an identified NEAR wallet. |  


### Milestone 2 — Token-gated verification
...
| 1. | Simple Content Management System | A simple CMS that will allow users to create, edit and publish digital content, such as text, images, and videos. The CMS will have a user-friendly interface that allows users to quickly and easily access all of the necessary tools for managing their content |  
| 2. | Access Criteria Management | Allow the admin to set different access criteria for the content, including which tokens and collections are allowed to be able to have access to the content |  
| 3. | Token Verification in wallet | This feature will verify the authenticity of the tokens stored in their wallet, ensuring that the tokens they hold are legitimate, and if so, display the content they are eligible to get. Our goal is that the verification process will be quick and seamless, ensuring that users can verify their tokens without interrupting their workflow. |  
