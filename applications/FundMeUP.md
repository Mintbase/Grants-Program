...
| 1d. | Discord - Community Building Techniques | a place to discuss strategies for increasing the size of the community |  
...
| 1g. | React App - Campaign Builder Progressive | area that handles the Progressive Campaign Builder with associated components |  
| 1h. | React App - Authenticated Management - Redeemable Assets | a place for users to browse their redeemables |  
| 1i. | React App - Authenticated Management - Campaign Management Owned | a place to manage campaigns that the end user began |  
| 1j. | React App - Authenticated Management - Campaign Management Funded | a place interact with campaigns that the end user has funded |  

### Milestone 3 - Consecration 
...
| Number | Deliverable | Specification |
| -----: | ----------- | ------------- |
| 0a. | NearWrapperContract - method | mintBaseOffer() - * accepts payment for a 'partitioned flow' asset, interacts with Mintbase Contract, leverages callback * |
| 0b. | NearWrapperContract - method  | batchMintBaseMint() - * mints partitioned assets for totality of campaign generation, interacts with Mintbase Contract, leverages callback * |
| 0c. | NearWrapperContract - method  | batchMintBaseList() - * lists partitioned assets onto the mintbase store associated with fundMeUP, interacts with Mintbase Contract, leverages callback * |
| 0d. | NearWrapperContract - method  | makeProgressiveOffer() - * accepts a contribution to a progressive campaign * |
| 0e. | NearWrapperContract - method  | mintProgressiveNFT() - * creates divisible NFT(s) associated with each contribution, interacts with escrow * |
| 0f. | NearWrapperContract - method  | batchTransferProgvessiveNFTS() - * handles the distribution of redeemables in the progressive flow * |
| 0g. | NearWrapperContract - method  | distributeProgressiveEscrow() - * handles the distribution of redeemables in the progressive flow * |
| 0h. | NearWrapperContract - method  | refundProgressiveEscrow() - * handles the refund of funds from campaigns that were not successful  * |
| 0i. | NearWrapperContract - method  | sendCashBackToListers() - * handles the distribution of funds from specified wallet   * |
| 0i. | Docker file with unit tests  | a docker file will be provided that runs tests against the methods above |
| 1a. | Report | Cash back to Listers Program - explanation of strategy around a specified treasury for sending near back to listers who are early adopters of the platform to cover their listing fees |
 1b. | Report | A cost benefit of analysis of the liability allowing users to associate their own images in campaigns vs. AI generated art associated with specific keywords. AI generative library research |
| 1c. | Report | Go to Market Strategy - explanation of strategies and strategic distribution of funding resources towards growing the fundMeUP community |
| 2a. | ReactApp Polish | work will be done to polish client side react components |
| 3a. | twitter posts | 10 tweets over the course of this time period to engage twitter population | 
| 3b. | facebook posts | 10 posts over the course of this time period to engage facebook population | 
...
| Number | Deliverable | Specification |
| -----: | ----------- | ------------- |
| (0?) | option 1: configure indexer to handle progressive flow NEAR chain operations
| (0?) | option 2: create local postgres instance to manage progressive flow campaigns (this is not ideal but will be explored). Whether or not campaign drafts should be stored on or off chain (or if they need to be stored at all will be considered) |  
| 0a. | React App - Dashboard | testnet results pulled from mintbase api and from Progressive flow contracts. |  
| 0b. | React App - Funding Flow Partitioned | interacts with contract and performs fulfillment for purchase |  
| 0c. | React App - Funding Flow Progressive | interacts with contract and performs fulfillment for purchase|
| 0d. | React App - Campaign Builder Progressive | interacts with contract and performs fulfillment for minting and listing |  
| 0e. | React App - Campaign Builder Progressive | interacts with contract and performs fulfillment for minting and listing |
| 0f. | React App - Authenticated Management - Redeemable Assets | user specific results from testnet |  
