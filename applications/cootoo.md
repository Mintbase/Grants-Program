...
| # | Deliverable | Specification |
| 0a.  | License | Unlicense |
| 0b.  | Documentation | Steps for setup and activation of the website, and for configuring it to use a specific contract instance on a specific network, will be covered in README.md & supported by Makefile targets.  Every HTML page component will be documented inline (HTML comments) with consistent naming. All styles will be documented inline (CSS comments) with consistent naming. The repo directory structure, the purpose of every included external module, and the purpose of every other repo file will also be documented in README.md |
| 0c.  | Testing Guide | The mockup web UI will be human-testable.  (At this stage we will be testing for completion of 1a-1f in both mobile & desktop contexts, and looking for possible design improvements.) |
| 0d.  | Docker | We will provide a Dockerfile that serves the Gallery UI.  |
| 1.  | Site Mockups | Working from provided design files, we will develop the HTML+CSS representation of all pages of the Stateless Gallery, and provide a mockup that demonstrates the appearance and basic UI behavior of the Gallery site in a browser. The site design will be realized in Bootstrap 5 using clearly demarked & decomposable components, styled only with external CSS.  The site design will include mobile and desktop layouts.  The mocked-up Gallery components will include: |
| 1a | single NFT page | Displays a single NFT in a styleable frame |
| 1b | multi NFT page | Displays a configurable number of NFTs in styleable frames |
| 1c | NFT detail page | Displays a single NFT in a styleable frame with its metadata, market data, and an optional buy-button.  |
| 1d | Artist detail page | Displays artist avatar and metadata, and allows paging through the complete set of works by that artist in the contract.  |
| 1e | Collector detail page | Lets the current user page through the complete set of works they own in the contract.  |
| 1f | Navigation bar | Enables navigation through all site pages, plus site identity and documentation.  |





### Milestone 4: Stateless Gallery Live Site: Gallery Components
...
| # | Deliverable | Specification |
| 0a.  | License | Unlicense |
| 0b.  | Documentation | Steps for setup and activation of the website, and for configuring it to use a specific contract instance on a specific network, will be covered in the README file of the repo & supported by Makefile targets. React components will have consistent naming & inline JSDoc documentation. The site navigation will include a documentation link that introduces the Stateless project and explains the user interface.  |
...
| 1.  | React Components | We will implement React components to integrate the interface components of Milestone 3 with a live Mintbase contract.  In this milestone we will implement components 1a, 1b, 1c and 1f |
| 2 | React Router | We will implement a React router to manage the site URL scheme, and implement a routing scheme for the above components.  |
| 3 | NFT Shuffle Algorithm | We will implement an algorithm for paging through an array of NFTs in a random order without repeating elements, then re-shuffling when the end is reached. (The state of the shuffled array will be preserved without the use of client-side cookies, to avoid GDPR notification requirements.) |
